...
def coeffs(bnms, bvals, cnms, kvals=None, kren=None, maxb=np.inf, verb=False):
    ...
    for i, c in enumerate(cnms):
        ...
        for b in bs:
            ...
            try:
                ...
                raise e
            if verb:
                print('coeff*' + b + '=', bvals[bnms.index(b)])
...
def sum_zbEFT(pkterms, calphas, pklin=None, verb=False):
    ...
    if verb:
        print('P_gg^l(k) = ', pkvals[-1], pkvals.shape)
...
def plks_zbEFT_fromterms(kvals, data, cnames, bvals, bnames,
    ...
    cas = coeffs(['1'] + bnames, [1.] + list(bvals), cnames,
                 dmonly, kvals, kren, maxb, verb=False)
...
def plks_zbEFT_fromfile(bvals, basepath='./' + tools.FBSNM, maxb=np.inf,
                        dmonly=False, kren=None, lintoo=False,
                        kvals=None, mlps=[0, 2, 4],
                        eft=tools.FEFT, lin=tools.FLIN, lincols=3):

    if dmonly and kren is None:
        raise Exception('If dmonly, need kren input!')

    # Get power spectra terms from the file
    mpls, kvals_f, data_f, names = tools.read_zbEFT(
        basepath, mlps=mlps, eft=eft, lin=lin)
    ...
    if kvals is not None and (len(kvals) != len(kvals_f) or not np.any(kvals, kvals_f)):
        ...
        nl_f = tools.count_mlps(kvals_f)
        nk_f = len(kvals_f) / nl_f
        nl = tools.count_mlps(kvals)
        nk = len(kvals) / nl

        # Make sure the requested number of multipoles is self-consistent
        if (nl != len(mlps) and nl != 1) or nl_f < nl:
            raise Exception(
                'The requested number of multipoles is inconsistent! -> ' + str([nl, len(mlps), nl_f]))
        elif nl == 1:
        ...
        for l in range(len(mlps)):
            ...
            try:
                data[nk * l:nk * (l + 1), :] = scipy.interpolate.interp1d(kvals_f[imin:imax],
                                                                          data_f[imin:imax, :], axis=0)(k)
            except ValueError as e:
                message = '\n You are asking for a range of [' + str(
                    np.min(k)) + ', ' + str(np.max(k))
                message += '], but we only have a range of [' + \
                    str(np.min(kvals_f[imin:imax])) + \
                    ', ' + str(np.max(kvals_f[imin:imax]))
    ...
    return plks_zbEFT_fromterms(kvals, data, names, bvals, maxb, dmonly, kren, lintoo, lincols)
...
def plks_zbEFT(pars, kvals, path=None):
    """ Calculates the power spectrum multipoles given a dictionary or ConfigObj of
        ...
        pars : dict or ConfigObj
            the configuration - see the global variables in tools.py for defaults
    ...
    bp = op.abspath(op.join(path, tools.FBSNM))
...
def run_zbEFTClassonly(config):
    """ Runs Pierre Zhang's EFT code in z-space with biased tracers
        ...
        config : str or configobj.ConfigObj or dict
                ...
                or a configobj.ConfigObj or a dictionary containing
                    the parameter names (predetermined - see tools.py) and values
    ...
    if type(config) == str:
        cclass, czbEFT, czbEFTw = tools.get_config(bigconfig_file=config)
    else:
        cclass, czbEFT, czbEFTw = tools.get_config(bigconfig=config)

    # Set it all up
    tools.setup_outputs(cclass, czbEFT, czbEFTw)  # Creates the folder
    tools.make_configfiles([cclass, czbEFT, czbEFTw],
    ...
    # Precision accuracy downgraded for faster computation -> OK for computing sigma8
    command = [czbEFTw['CLASS_exe'], czbEFTw['CLASS_configf']]
    tools.runcommand(command, czbEFTw['logfile'],
                     czbEFT['PathToLinearPowerSpectrum'],
                     cwd=op.dirname(czbEFTw['CLASS_exe']))
...
def run_zbEFT(config):
    """ Runs Pierre Zhang's EFT code in z-space with biased tracers
        ...
        ------
        config : str or configobj.ConfigObj or dict.
                If a string, it is the path to the full config file.
                If a configobj.ConfigObj or a dictionary, it contains
                the parameter names (predetermined - see tools.py) and values.
    ...
    # Set it all up. We actually don't need the config files
    tools.setup_outputs(cclass, czbEFT, czbEFTw)
    tools.make_configfiles([cclass, czbEFT, czbEFTw],
                           ['CLASS_configf', 'zbEFT_configf', 'zbEFTw_configf'], indfmeta=2)

    # Run CLASS (someday maybe use classy)
    command = [czbEFTw['CLASS_exe'],
               czbEFTw['CLASS_configf'], czbEFTw['CLASS_pre']]
    # print(command)
    # print(cclass)
    # print(czbEFT)
    # print(czbEFTw)
...
if __name__ == '__main__':
    ...
    print('Wrapper took ' + str(round(time.time() - t0, 2)) +
          ' seconds to run EFT code.\nResults of the zbEFT run saved to: ' + path)
