...
def testcs(arraytheta, nrank):
    ...
    for i, theta in enumerate(arrayred):
        ...
        if i % 100 == 0:
            np.save("/scratch/users/gda/Pk/temp/Plin_rank%si%s.npy" % (str(nrank), str(i)), np.array(allPlin))
            np.save("/scratch/users/gda/Pk/temp/Ploop_rank%si%s.npy" % (str(nrank), str(i)), np.array(allPloop))
    np.save("/scratch/users/gda/Pk/Plin_rank%s.npy" % str(nrank), np.array(allPlin))
    np.save("/scratch/users/gda/Pk/Ploop_rank%s.npy" % str(nrank), np.array(allPloop))
    return


testcs(thetatab, rank)
